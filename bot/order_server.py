...
class OrderManagementServer:
    ...
    def __init__(self):
        ...
        self.status_messages = {
            ...
            OrderStatus.CONTACTED: "🔄 سفارش شما درحال پیگیری است. لطفا صبور باشید",
            OrderStatus.CONFIRMED: "✅ سفارش شما توسط تیم پشتیبانی دکوتین تایید شد. از اعتماد شما ممنونیم",
            OrderStatus.PREPARING: "🔧 سفارش شما در حال آماده‌سازی است.",
            OrderStatus.READY: "📦 سفارش شما آماده ارسال شد!",
            OrderStatus.SHIPPED: "🚚 سفارش شما تکمیل و ارسال شد. از صبوری و همکاری شما ممنونیم",
    ...
    async def _notify_customer(self, user_id: int, status: str, order_id: str, admin_name: str = ""):
        ...
        try:
            ...
            if status == OrderStatus.CANCELLED:
                ...
                # برای وضعیت shipped نام مشتری را اضافه کنیم
                if status == OrderStatus.SHIPPED and order_data and order_data.get('customer'):
                    customer_name = order_data['customer'].get('name', 'مشتری گرامی')
                    message = f"جناب آقای {customer_name} عزیز سفارش شما تکمیل و ارسال شد. از صبوری و همکاری شما ممنونیم"
    ...
    async def get_daily_orders(self, date: str = None) -> List[Dict]:
        """دریافت سفارشات روزانه (برای ادمین)"""
        if date is None:
        ...
        try:
            for filename in os.listdir(self.orders_dir):
                if filename.startswith("order_") and filename.endswith(".json"):
                    ...
                    if order_data:
                        ...
                        created_date = order_data.get("created_at", "")[:10]  # فقط بخش تاریخ
            ...
            logger.error(f"خطا در دریافت سفارشات روزانه: {e}")
            return []
